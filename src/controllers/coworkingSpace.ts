...
export const getCoworkingSpaceTotalReservation: RequestHandler = async (req, res) => {
  await dbConnect();
  try {
    const result = (
...
export const getCoworkingSpaceFrequency: RequestHandler = async (req, res) => {
  await dbConnect();
  try {
    const result = (
      await Reservation.aggregate<{ data: { label: string; data: number[] }[]; label: string[] } | undefined>(
        [
          { $match: { coworkingSpace: mongoose.Types.ObjectId.createFromHexString(req.params.id) } },
          {
            $set: {
