...
import BanAppeal, { BanAppealType, Comment } from "../models/BanAppeal.js";
...
export const getBanAppeal: RequestHandler = async (req, res) => {
  try {
    await dbConnect();
    const banAppeal = (
      await BanAppeal.aggregate<
...
export const createBanAppeal: RequestHandler = async (req, res) => {
  try {
    await dbConnect();
...
export const createBanAppealComment: RequestHandler = async (req, res) => {
  try {
    await dbConnect();
    const banAppeal = await BanAppeal.findByIdAndUpdate(
      req.params.appeal,
      { $push: { comment: { ...req.body, user: req.user!.id } } },
...
export const updateBanAppeal: RequestHandler = async (req, res) => {
  await dbConnect();
  try {
    ...
    if (banAppeal && banAppeal.resolveStatus == "pending") {
      ...
      const [updatedBanAppeal, updatedBanIssue] = await Promise.all([
        BanAppeal.findByIdAndUpdate(
          req.params.appeal,
          { resolveStatus, resolvedAt },
          { new: true, runValidators: true }
        ),
